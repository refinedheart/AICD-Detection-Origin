...
class ComputeLoss:
    ...
    def get_fgfi_mask(self, batch_size, feat_h, feat_w, stride, targets, anchors_layer):
        ...
        grid_y, grid_x = torch.meshgrid(
            [torch.arange(feat_h, device=self.device), torch.arange(feat_w, device=self.device)], indexing="ij"
        )
        ...
        anchors_grid = torch.cat(
            ...
            dim=-1,
        ).view(-1, 4)  # Flatten [N_all, 4]

        # 转为 x1y1x2y2
        anchors_xyxy = torch.zeros_like(anchors_grid)
        anchors_xyxy[:, 0], anchors_xyxy[:, 1] = (
            anchors_grid[:, 0] - anchors_grid[:, 2] / 2,
            anchors_grid[:, 1] - anchors_grid[:, 3] / 2,
        )
        anchors_xyxy[:, 2], anchors_xyxy[:, 3] = (
            anchors_grid[:, 0] + anchors_grid[:, 2] / 2,
            anchors_grid[:, 1] + anchors_grid[:, 3] / 2,
        )
    ...
    def __call__(self, p, targets, imgs=None):  # predictions, targets
        ...
        if self.distill_ok and self.model.training and imgs is not None:
            ...
            if len(student_feats) == len(teacher_feats):
                for i, (s_feat, t_feat, projector) in enumerate(
